...
const normalize = (s) =>
  (s || "")
    .toUpperCase()
    .normalize("NFD")
    // Quita acentos/diacríticos (compatible cross-browser, evita \p{Diacritic})
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^A-Z0-9 ]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
// MRZ: intenta extraer nombre en APELLIDOS<<NOMBRES
...
function compareAgainstBase(docCanon, nameCanon) {
  ...
  // Exacto o similar (full / short)
  for (const r of list) {
    if (nameCanon === r.full || nameCanon === r.short) {
      return setResult(true, "Documento y nombre coinciden (exacto).");
    }
    const s1 = similarEnough(nameCanon, r.full);
    const s2 = similarEnough(nameCanon, r.short);
    if (s1.ok || s2.ok) {
      return setResult(true, "Documento coincide y el nombre es similar (tolerancia aplicada).");
    }
  }
  const candidates = list.map(r =>
    [r.originalRow.nombre, r.originalRow.apellido1, r.originalRow.apellido2].filter(Boolean).join(" ")
  ).join(" · ");
...
async function ocrImage(imageBitmap) {
  ...
  try {
    ...
    // Nombre: MRZ preferente; si no hay, línea más larga en mayúsculas
    ...
    if (!nameLine) {
      ...
      nameLine = t2.split(/\r?\n/)
                   .map(l => l.trim())
                   .filter(l => l && /^[A-ZÁÉÍÓÚÑÜ\s]+$/.test(l))
...
startCamBtn.addEventListener("click", async () => {
  try {
    ...
    alert("No se pudo acceder a la cámara. Prueba Chrome (Android) o Safari (iOS) y permite el acceso.");
  }
});
